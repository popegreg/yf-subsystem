            ...
            if(empty($selecteditem))
            {
                ...
                if (count($detailsList) > 0) {
                    ...
                    foreach ($detailsList as $key => $row) {
                        if (strlen($row[1]) == 15 || substr($row[1],0,5) == 'NECL2' || substr($row[1],0,2) == 'ES') {
                            $unitprice = $row[4];
                        } else {
            ...
            else
            {
                ...
                if (count($detailsList) > 0) {
                    ...
                    foreach ($detailsList as $key => $row) {
                        ...
                        if (strlen($row[1]) == 15 || substr($row[1],0,5) == 'NELC2' || substr($row[1],0,2) == 'ES') {
                            $unitprice = $row[4];
                        } else {
                               $code = $row[1];
                             $isInXtank = DB::connection($this->mssql)->table('XTANK')
    ...
    private function insertPackingDetails($detailsList, $selecteditem, $action)
    {
        ...
        try
        {
            if(count($detailsList) >= 1)
            {
                ...
                {
                    if (strlen($detailsRow[1]) == 15 || substr($detailsRow[1],0,5) == 'NECL2' || substr($detailsRow[1],0,2) == 'ES') {
